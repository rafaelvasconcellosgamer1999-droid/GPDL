    ...
    public function apiIndex(Request $request)
    {
        ...
        if ($search !== '') {
            $q->where(function (Builder $qq) use ($search) {
                $qq->whereRaw("cnj ILIKE ?", ["%{$search}%"])
                    ->orWhereRaw("municipio ILIKE ?", ["%{$search}%"])
                    ->orWhereHas('acao', fn($r) => $r->whereRaw("nome ILIKE ?", ["%{$search}%"]))
                    ->orWhereHas('assunto', fn($r) => $r->whereRaw("nome ILIKE ?", ["%{$search}%"]));
            });
        }
