...
export default function Encerrados({ processos, filters, procuradores }: Props) {
  ...
  const onApply = () => {
    router.get('/processos/encerrados', f, { preserveState: true, preserveScroll: true });
  };

  const onClear = () => {
    setF({ responsavel_id: '', order: 'prazo_asc', per_page: '10' });
    router.get('/processos/encerrados', {}, { preserveState: true });
  };

  return (
    <GPDLLayout breadcrumbs={[{ title: 'Processos', href: '/processos/ativos' }, { title: 'Finalizados', href: '/processos/encerrados' }]}>
      <Head title="Processos - Finalizados" />
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-(--text-strong)">Processos</h1>
        <p className="mt-1 text-sm text-(--text-muted)">Histórico de arquivados e concluídos</p>
      </div>

      <div className="rounded-lg bg-(--surface-card) p-6 shadow-sm">
        <h2 className="mb-4 text-lg font-semibold text-(--text-strong)">Finalizados</h2>
