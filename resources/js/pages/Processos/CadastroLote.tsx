  ...
  { title: 'Cadastro em lote', href: '' },]}>
      ...
      <div className="mt-4 rounded-lg bg-[var(--surface-card)] p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[var(--text-strong)]">Cadastro em lote</h2>
        ...
        <form onSubmit={submit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-[var(--text-muted)]">Modelo</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-[var(--text-muted)]">Cole o texto completo do processo</label>
          ...
          <div className="flex items-center gap-3">
            <button type="submit" disabled={processing} className="flex items-center gap-2 rounded-lg bg-[var(--brand-700)] px-4 py-2 text-white hover:bg-[var(--brand-600)] disabled:opacity-50">
              {processing ? 'Importando...' : 'Adicionar processo(s)'}
            </button>
            <button type="button" onClick={() => reset()} className="flex items-center gap-2 rounded-lg bg-[var(--surface-muted)] px-4 py-2">Limpar campos</button>
          </div>
        </form>
      </div>
    </GPDLLayout>
